namespace LAB_6_V2
{
    ...
    public class VoiceAssistant
    {
        ...
        {
            voiceType = "Жіночий"; // Базовий голос для прикладу.
            language = "Українська"; // Мова взаємодії з користувачем.
            isActive = true; // У V2 асистент одразу активний.
        }
        ...
        public string InterpretCommand(string command)
        {
            if (command == "status")
            {
                return Speak("Показую поточний статус холодильника."); // Сценарій перевірки стану.
            }

            if (command == "cool")
            {
                return Speak("Запускаю цикл охолодження."); // Сценарій запуску охолодження.
            }
