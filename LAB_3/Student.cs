...
public class Student
{
    ...
    /// <summary>
    /// Метод UpdateSubject, який приймає рядок newSubject і оновлює назву предмету, який вивчає студент, на це нове значення. Цей метод використовується для зміни назви предмету, який вивчає студент, відповідно до переданого аргументу.
    /// </summary>
    public class DiplomaProject
    {
        ...
        /// <summary>
        /// Конструктор за замовчуванням для класу DiplomaProject, який ініціалізує всі поля класу значеннями за замовчуванням. Поля ініціалізуються порожніми рядками або нулями відповідно до їх типів даних. Цей конструктор використовується для створення об'єкта DiplomaProject з початковими значеннями, які можуть бути змінені пізніше за допомогою властивостей або методів класу.
        /// </summary>
        ...
        /// <summary>
        /// Властивості для доступу до полів класу DiplomaProject: NameOfTheme, NumOfCompletedAlgorithms, NameOfTeacher, DificultyOfTheme та Mark. Вони дозволяють отримувати та встановлювати значення відповідних полів.
        /// </summary>
        ...
        /// <summary>
        /// Метод ChooseTheme, який дозволяє студенту вибрати тему для дипломного проекту. Метод запитує у користувача ключове слово для пошуку теми, а потім зчитує всі теми з файлу "themes.txt" та виводить ті, які містять введене ключове слово. Якщо файл не знайдено, виводиться повідомлення про помилку. Якщо теми за таким ключовим словом не знайдено, також виводиться відповідне повідомлення. Цей метод використовується для допомоги студенту у виборі теми для його дипломного проекту на основі доступних тем у файлі.
        /// </summary>
        ...
        public void ChooseTheme()
        {
            ...
            if (!File.Exists(filePath))// Якщо файл не існує, виводимо повідомлення про помилку та повертаємося з методу
            {
                Console.WriteLine("Помилка: Файл із темами не знайдено!");
                return;
            }
            string[] linesArray = File.ReadAllLines(filePath);// Зчитуємо всі рядки з файлу у масив рядків

            Console.WriteLine("\nЗнайдені теми:");
            bool found = false;// Флаг для відстеження, чи знайдено хоча б одну тему, що відповідає ключовому слову

            foreach (string line in linesArray)// Проходимо по кожному рядку в масиві рядків
            {
                // Перевіряємо, чи містить рядок ключове слово
                // ToLower() додаємо для того, щоб пошук не залежав від регістру (великі/малі літери)
                if (line.ToLower().Contains(themeName.ToLower()))// Якщо рядок містить ключове слово, виводимо його та встановлюємо флаг found в true
                {
            ...
            if (!found)// Якщо після проходження всіх рядків флаг found залишився false, це означає, що теми за таким ключовим словом не знайдено, і ми виводимо відповідне повідомлення
            {
