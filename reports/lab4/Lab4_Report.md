...
Метою лабораторної роботи є поглиблення навичок об'єктно-орієнтованого програмування мовою C# через розроблення консольної моделі освітнього процесу з використанням успадкування, абстрактного класу, інтерфейсів і стандартних механізмів порівняння та перебору об'єктів.

У роботі необхідно опанувати:

- створення базових і похідних класів;
- використання конструкторів базового та похідного класів;
- реалізацію конструкторів за замовчуванням, конструкторів з параметрами та конструкторів копіювання;
- використання ключового слова `base` для ініціалізації успадкованої частини об'єкта;
- створення абстрактного класу як узагальненої сутності предметної області;
- застосування захищених (`protected`) конструкторів у базовому класі;
- реалізацію власного інтерфейсу `IPerson`;
- демонстрацію поліморфізму через посилання інтерфейсного типу;
- реалізацію стандартних інтерфейсів .NET: `IComparable<Student>`, `IComparer<Student>`, `IEnumerable` та `IEnumerator`;
- сортування колекції студентів за рейтингом і кількістю виконаних робіт;
- перебір групи студентів через `foreach`;
- збереження стану об'єктів і повного протоколу консольної взаємодії у текстовий файл.
...
- зробити `Person` абстрактним базовим класом;
...
`IPerson` не є окремим учасником предметної області. Це інтерфейс, який фіксує спільний контракт для об'єктів, що мають властивості `Name` і `SubjectName`. Він потрібний для демонстрації поліморфізму: об'єкт конкретного класу можна розглядати через узагальнений інтерфейсний тип.

### Person

`Person` представляє спільну частину викладача і студента. У предметній області немає потреби створювати просто "людину" без уточнення ролі, тому клас є абстрактним. Він містить ім'я та дисципліну, а також метод `DisplayInfo()`, який може бути використаний для виведення базової інформації.
...
Клас `Person` зроблено абстрактним, тому що в межах моделі не існує самостійного об'єкта "людина" без ролі. Реальні об'єкти програми - це або викладач, або студент. Абстрактний клас дозволяє винести спільні поля та властивості в одне місце, але не дозволяє створювати екземпляр базового класу напряму.

Конструктори `Person` мають модифікатор `protected`. Це означає, що їх можуть викликати лише похідні класи. Такий підхід відповідає предметній області: створювати потрібно `Teacher` або `Student`, а не абстрактний `Person`.
...
- `Teacher --|> Person` - успадкування від абстрактного класу;
- `Student --|> Person` - успадкування від абстрактного класу;
...
- `Person.cs` - містить абстрактний базовий клас `Person`, його закриті поля, захищені конструктори, властивості та метод `DisplayInfo()`.
...
`IPerson` - інтерфейс для узагальнення спільних властивостей людини.
...
Інтерфейс використовується для демонстрації поліморфізму. У `Program.cs` об'єкт `Teacher` присвоюється змінній типу `IPerson`, після чого через неї зчитується ім'я викладача.

### Person

`Person` - абстрактний базовий клас для учасників освітнього процесу.
...
Абстракція реалізована через клас `Person`. Він містить спільні характеристики людини, але не описує конкретну роль у навчальному процесі. Тому клас оголошено як `abstract`.
...
Поліморфізм продемонстровано через інтерфейс `IPerson`. У програмі створюється викладач:
...
Після цього об'єкт `Teacher` використовується через посилання типу `IPerson`. Це показує, що конкретний об'єкт можна розглядати через узагальнений контракт.
...
Було реалізовано абстракцію через `abstract class Person`. Це дозволило точно відобразити предметну область: у програмі не потрібно створювати об'єкт простої людини, але потрібно мати спільну основу для викладача і студента. Захищені конструктори `Person` гарантують, що базова частина об'єкта ініціалізується лише через похідні класи.

Поліморфізм було продемонстровано через інтерфейс `IPerson`. Об'єкт класу `Teacher` було присвоєно змінній типу `IPerson`, що показує можливість працювати з конкретними об'єктами через узагальнений контракт.

Окрему увагу приділено стандартним інтерфейсам .NET. `IComparable<Student>` використано для природного сортування студентів за рейтингом. `IComparer<Student>` використано для альтернативного сортування за кількістю виконаних робіт. `IEnumerable` та `IEnumerator` дали змогу перебирати групу студентів через `foreach`.
